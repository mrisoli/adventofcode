...
calcCircuits :: TableCircuit -> TableCircuit
calcCircuits (t, c) =
    let v = Map.lookup "a" t
     in case v of
          Just i -> (t, c)
...
reset :: TableCircuit -> Int -> TableCircuit
reset (t,c) i = (insert "b" i t, insert "b" (StmtValue (ExpValue i)) c)

getAns :: TableCircuit -> Int
getAns (t, _) =
    let v = Map.lookup "a" t
     in case v of
          Just i -> i
