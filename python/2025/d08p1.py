...
def connect_all(points, connections):
    d = distances(points)[:connections]
...
def largest_circuits(circuits):
    return sorted(list(map(len, circuits)), reverse=True)[:3]

f = list(map(tuple, rows_of_int(8)))
connections = 1000 if len(f) >= 1000 else 10
circuits = connect_all(f, connections)
l = largest_circuits(circuits)
print(prod(l))
